...
describe('setCookie', () => {
  test('Basic convert "set-cookie" response header to Map<string, string>', () => {
    ...
    const result = setCookie(cookies);
    ...
    expect(result.get('value=moskva;')).toBe(
      'value=moskva; expires=Tue, 04-Jan-2022 11:23:11 GMT; Max-Age=86400; path=/; domain=.avito.ru; secure; HttpOnly; SameSite=Lax',
    );
  });
  ...
  test('Incorrect strings', () => {
    const result = setCookie(['abc', 'bla bla bla']);
    expect(result).toBeInstanceOf(Map);
    expect(result.size).toBe(0);
  });
});
...
describe('getCookie', () => {
  test('Convert Map<string, string> to "cookie" request header', () => {
    ...
    test.set(
      'anotherValue=2;',
      'anotherValue=2; expires=Tue, 04-Jan-2022 11:23:11 GMT; Max-Age=86400; path=/; domain=.avito.ru; secure; HttpOnly; SameSite=Lax',
    );
